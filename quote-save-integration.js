/* ============================================================
   BuildQuotes — Save/Load Integration
   Connects the quote form to BuildAuth for persistence.
...
(function () {
  ...
  function readFormData() {
    ...
    return {
      ...
      to_city:      v("toCity"),
      to_state:     v("toState"),
      quote_number: v("quoteNumber"),
      quote_date:   v("quoteDate"),
  ...
  function loadFormData(data) {
    ...
    setVal("quoteNumber", data.quote_number);
    setVal("quoteDate",   data.quote_date);
    ...
    if (data.line_items && data.line_items.length > 0) {
      // Set global lineItems if accessible
  ...
  function buildTitle(data) {
    var parts = [];
    if (data.quote_number) parts.push(data.quote_number);
  ...
  function init() {
    ...
    BuildAuth.onAuthChange(function (user) {
      ...
      var saveBtn = document.getElementById("bqs-save-btn");
      var hint = document.getElementById("bqs-save-hint");

      if (user) {
        if (saveBtn) saveBtn.style.display = "";
        if (hint) hint.style.display = "none";
        if (panel) { panel.style.display = ""; loadSavedQuotes(); }
      } else {
  ...
  function injectSaveButton() {
    ...
    saveBtn.type = "button";
    saveBtn.id = "bqs-save-btn";
    saveBtn.style.cssText = "display:none;background:rgba(108,99,255,0.15);border:1px solid rgba(108,99,255,0.3);color:#a5b4fc;padding:0.75rem 1.25rem;border-radius:12px;font-weight:600;font-size:0.95rem;cursor:pointer;transition:all 0.2s;white-space:nowrap;font-family:inherit;";
    saveBtn.textContent = "💾 Save";
    saveBtn.title = "Save this quote to your account";
    saveBtn.addEventListener("mouseenter", function () { saveBtn.style.background = "rgba(108,99,255,0.25)"; });
    saveBtn.addEventListener("mouseleave", function () { saveBtn.style.background = "rgba(108,99,255,0.15)"; });
    ...
    hint.type = "button";
    hint.id = "bqs-save-hint";
    hint.className = "bea-save-hint";
    hint.textContent = "💾 Sign in to save your quotes";
  ...
  async function handleSave() {
    var btn = document.getElementById("bqs-save-btn");
    ...
    var docId = await BuildAuth.saveDocument("quote", title, data, {
  ...
  function injectSavedPanel() {
    var form = document.querySelector(".form-card, .quote-form, #quoteForm");
    if (!form) {
      // Try finding the first section-like container
      form = document.querySelector("main") || document.querySelector(".container");
    }
    if (!form) return;

    var panel = document.createElement("div");
    panel.id = "bqs-saved-panel";
    panel.style.cssText = "display:none;margin-bottom:2rem;background:rgba(108,99,255,0.04);border:1px solid rgba(108,99,255,0.12);border-radius:16px;padding:1.5rem;";
    panel.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">' +
        '<h3 style="margin:0;font-size:1rem;font-weight:700;color:rgba(255,255,255,0.85);">📋 Your Saved Quotes</h3>' +
        '<button id="bqs-refresh" style="background:none;border:none;color:rgba(255,255,255,0.4);cursor:pointer;font-size:0.85rem;">↻ Refresh</button>' +
      '</div>' +
      '<div id="bqs-list" style="display:flex;flex-direction:column;gap:0.5rem;"></div>';

    form.parentElement.insertBefore(panel, form);

    document.getElementById("bqs-refresh")?.addEventListener("click", loadSavedQuotes);
  }

  async function loadSavedQuotes() {
    var list = document.getElementById("bqs-list");
    if (!list) return;

    list.innerHTML = '<div style="color:rgba(255,255,255,0.3);font-size:0.85rem;">Loading...</div>';

    var docs = await BuildAuth.loadDocuments("quote");

    if (docs.length === 0) {
      list.innerHTML = '<div style="color:rgba(255,255,255,0.3);font-size:0.85rem;">No saved quotes yet. Create a quote and click 💾 Save.</div>';
      return;
    }
    ...
    docs.forEach(function (doc) {
      ...
      loadBtn.style.cssText = "background:rgba(108,99,255,0.15);border:1px solid rgba(108,99,255,0.25);color:#a5b4fc;padding:5px 12px;border-radius:8px;font-size:0.8rem;cursor:pointer;font-family:inherit;";
      loadBtn.textContent = "Load";
      loadBtn.addEventListener("click", function (e) { e.stopPropagation(); loadQuote(doc.id); });
      ...
      delBtn.addEventListener("click", function (e) {
        ...
        if (confirm("Delete this saved quote?")) {
          BuildAuth.deleteDocument(doc.id).then(function () { loadSavedQuotes(); });
        }
      });
  ...
  async function loadQuote(docId) {
    var doc = await BuildAuth.getDocument(docId);
    if (!doc || !doc.formData) { alert("Could not load quote."); return; }
